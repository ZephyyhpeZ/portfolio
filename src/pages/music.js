...
const ImageBackgroundDetector = ({ imageUrl, hex }) => {
  ...
  const darkenedColor = (r, g, b, factor) => {
    // Adjust the factor to control the darkness (0.5 for example)
...
function Music() {
  ...
  return (
    <div
      className="flex flex-col justify-center items-center h-screen w-screen gap-5 overflow-y-hidden"
      style={{
    ...
    >
      ...
      <div>
        <m.input
          ...
          placeholder="Search..."
          whileTap={{ scale: 1.1 }}
          className="lg:w-[200px] lg:h-[35px] bg-transparent border rounded-2xl px-2"
        />
      </div>
      <div
